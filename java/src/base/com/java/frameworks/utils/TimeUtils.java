...
public class TimeUtils {
	...
	public static final String DATE_FORMAT_LOG = "ddMMyyyy";// 15
	...
	public static String getDateStringForLog() {
		return getDateStringForLog(null);
	}

	public static String getDateStringForLog(Date date) {
		// String dateString;
		if (date == null)
			date = Calendar.getInstance().getTime();
		DateFormat dateFormat = new SimpleDateFormat(DATE_FORMAT_LOG);
		String strDate = dateFormat.format(date);
		logger.debug("Converted String: " + strDate);

		return strDate;
	}

	public static String getDecodeDateStringForLog(String strDate,
			String charAdd) {
		// String dateString;

		logger.debug("Decoded String: " + strDate);
		String result = strDate.substring(0, 2) + charAdd
				+ strDate.substring(2, 4) + charAdd
	...
	public static Date getCurrentTime() {
		 ...
		 * 
		 * Logs.println("Time zone: " + timeZone.getID()); Logs.println(
		 * "default time zone: " + TimeZone.getDefault().getID());
		 * Logs.println();
		 * 
		 * Logs.println("UTC:     " +
		 * simpleDateFormat.format(calendar.getTime())); Logs.println(
		 * "Default: " + calendar.getTime()); return
	...
	public static void sleepInMinutes(int minutes, String message,
			ITimeLog iTimeLog) {
		try {
			String log = "(pleep " + minutes + " minutes) -->message:"
					+ message;

			if (iTimeLog != null) {
				iTimeLog.showLog(log);
			} else {
				if (logger.isDebugEnabled()) {
					logger.debug(log);
				}
	...
	public static void sleepInSeconds(int seconds, String message,
		...
		// return;
		try {
			String log = "(pleep " + seconds + " seconds) -->message:"
					+ message;
	...
	public static Date fromUnixTimestamp(long timeinfb) {
		Date time = new Date(timeinfb * 1000);
